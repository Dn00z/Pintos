...
thread_set_priority (int new_priority) 
{
  ...
  if(list_empty( &thread_current()->donors)){
    ...
    struct thread *max_t = list_entry(list_front(&thread_current()->donors), struct thread, donor_elem);
    curr_thread->priority = max_t->priority;
  }

  if(new_priority > curr_thread->priority){
    curr_thread->priority = new_priority;
  }
  ...
  if(!list_empty(&ready_list))
  {
    ...
    if(f_thread->priority > curr_thread->priority)
      thread_yield();
  }
...
bool compare_priorities (const struct list_elem *a, const struct list_elem *b, void *aux){
